...
export function ProductCatalog({items}:{items:Product[]}){
  ...
  return <>
    ...
    <section className="catalog-list product-grid">{filtered.map((product,index)=><article key={product.slug}><a className="catalog-image-link" href={`/products/${product.slug}`} aria-label={`${product.name} 상세 정보 보기`}>{product.image&&<img className="catalog-image" src={product.image} alt={`${product.name} 제품 이미지`} loading="lazy" decoding="async"/>}<span>이미지 클릭하여 상세보기</span></a><small>0{index+1} · {product.category}</small><h2>{product.name}</h2><p>{product.summary}</p><div>{(product.uses??[]).map(value=><span key={value}>{value}</span>)}</div><a href={`/products/${product.slug}`}>상세 정보 <ArrowRight/></a></article>)}</section>
