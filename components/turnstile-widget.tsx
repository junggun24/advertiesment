...
export function TurnstileWidget({
  ...
  action: 'inquiry' | 'admin_login';
}) {
  const container = useRef<HTMLDivElement>(null);
  const widgetId = useRef<string | null>(null);
  const [scriptReady, setScriptReady] = useState(
    Boolean(globalThis.window?.turnstile),
  );
  ...
  useEffect(() => {
    ...
    void fetch('/api/security/config', { cache: 'no-store' })
      .then(
        async (response) =>
          response.json() as Promise<{ siteKey?: string; required?: boolean }>,
      )
      .then((config) => {
        if (!active) return;
        ...
        if (config.required !== false && !config.siteKey) {
          setError(
            '보안 검증을 준비하고 있습니다. 잠시 후 다시 시도해 주세요.',
          );
        }
      })
      .catch(() => {
  ...
  const renderWidget = useCallback(() => {
    if (!container.current || !siteKey || !window.turnstile || widgetId.current)
      return;
  ...
  if (!required && !siteKey) {
    return (
      <input type="hidden" name="turnstileToken" value="local-development" />
    );
  }
