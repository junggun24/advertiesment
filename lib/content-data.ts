...
export async function getContentRecords<T>(type:ContentType):Promise<ContentItem<T>[]>{
  try{
    const data=await dbApi(`/content?type=${type}`) as {items:ContentItem<T>[]};
...
export async function getContentItems<T>(type:ContentType):Promise<T[]>{
  ...
  return records.map(item=>{
    const content={...(item.data as object),slug:item.slug} as Record<string,unknown>;
    if(type==='product'||type==='case')content.image=resolveContentImage(content);
    return content as T;
  });
}

export function mergeBySlug<T extends {slug:string}>(fallback:T[],saved:T[]):T[]{
