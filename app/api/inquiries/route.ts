...
export async function POST(request: Request) {
  ...
  const privacyConsent = asText(body.privacyConsent) === 'agreed';
  const challenge = await verifyTurnstile(
    request,
    asText(body.turnstileToken),
    'inquiry',
  );
  ...
  try {
    rawAttribution = JSON.parse(asText(body.attribution) || '{}');
  } catch {
    rawAttribution = {};
  }
  ...
  if (!name || !organization || !contact || !message) {
    return Response.json(
      { ok: false, message: '필수 정보를 모두 입력해 주세요.' },
      { status: 400 },
    );
  }
  if (!privacyConsent) {
    return Response.json(
      { ok: false, message: '개인정보 수집 및 이용 동의가 필요합니다.' },
      { status: 400 },
    );
  }
  if (
    ...
    return Response.json(
      { ok: false, message: '입력 가능한 글자 수를 초과했습니다.' },
      { status: 400 },
    );
  }
  const allowed = [
  ...
  if (files.length > 5)
    return Response.json(
      { message: '첨부파일은 최대 5개까지 가능합니다.' },
      { status: 400 },
    );
  if (files.some((file) => file.size > 10 * 1024 * 1024))
    return Response.json(
      { message: '파일 한 개당 최대 크기는 10MB입니다.' },
      { status: 400 },
    );
  ...
  try {
    const inquiry = (await dbApi('/inquiries', {
      method: 'POST',
      body: JSON.stringify({
        name,
        organization,
    ...
    try {
      for (const file of files) await saveUpload(file, 'inquiry', inquiry.id);
    } catch (error) {
      await dbApi(`/inquiries/${inquiry.id}`, { method: 'DELETE' });
      throw error;
    }
    logInfo('inquiry.created', {
...
export async function GET(request: Request) {
  ...
  try {
    return Response.json(await dbApi('/inquiries'));
  } catch (error) {
    return Response.json(
      {
        message:
...
export async function PATCH(request: Request) {
  ...
  if (
    !Number.isInteger(id) ||
    !['new', 'contacting', 'quoted', 'contracted', 'closed'].includes(status)
  ) {
    return Response.json(
  ...
  try {
    await dbApi(`/inquiries/${id}`, {
      method: 'PATCH',
      body: JSON.stringify({ status, assignee, memo }),
    });
    return Response.json({ ok: true });
  } catch (error) {
    return Response.json(
...
export async function DELETE(request: Request) {
  ...
  if (!Number.isInteger(id))
    return Response.json(
      { message: '올바르지 않은 문의입니다.' },
      { status: 400 },
    );
  try {
    await dbApi(`/inquiries/${id}`, { method: 'DELETE' });
    return Response.json({ ok: true });
  } catch (error) {
    return Response.json(
      {
