...
import { normalizeEditorPayload } from '@/lib/editor-sanitize';
...
export async function PUT(
  ...
  try {
    const result = await dbApi(`/content/${id}`, {
      method: 'PUT',
      body: JSON.stringify(normalizeEditorPayload(await request.json())),
    });
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const all = (await dbApi('/content')) as {
      items?: Array<{ id: number; type?: unknown; slug?: unknown }>;
    };
