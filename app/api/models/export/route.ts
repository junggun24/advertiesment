...
export async function GET(request: Request) {
  ...
  try {
    const result = await cloudflareEnv().DB.prepare('SELECT * FROM display_models ORDER BY category, sort_order, id').all<Record<string, unknown>>();
    const bytes = await buildModelWorkbook(result.results.map(normalizeDisplayModel));
    ...
    return Response.json({ message: error instanceof Error ? error.message : '엑셀을 만들지 못했습니다.' }, { status: 503 });
  }
}
