...
export async function GET(request: Request) {
  try {
    ...
    const result = await cloudflareEnv()
      .DB.prepare(
        `SELECT * FROM display_models ${where} ORDER BY sort_order, id`,
      )
      .all<Record<string, unknown>>();
    return Response.json(
      { models: result.results.map(normalizeDisplayModel) },
      {
        headers: {
          'cache-control': includeAll ? 'no-store' : 'public, max-age=60',
        },
      },
    );
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    const values = displayModelValues(
      (await request.json()) as Record<string, unknown>,
    );
    const result = await cloudflareEnv()
      .DB.prepare(
        `INSERT INTO display_models (${displayModelFields.join(',')}) VALUES (${displayModelFields.map(() => '?').join(',')})`,
      )
      .bind(...values)
      .run();
    const row = await cloudflareEnv()
      .DB.prepare('SELECT * FROM display_models WHERE id=?')
