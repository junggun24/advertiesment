...
export default function CasesPage(){
  ...
  return <main className="catalog-page">
    ...
    <section className="catalog-list case-list">{filtered.map((c,i)=><article key={c.slug}><small>CASE 0{i+1}</small><h2>{c.title}</h2><p><MapPin/> {c.place} · {c.purpose}</p><span>{c.status}</span><a href={`/cases/${c.slug}`}>구성 보기 <ArrowRight/></a></article>)}</section>
  </main>
}
