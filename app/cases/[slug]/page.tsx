...
export default async function CaseDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="detail-page">
    <div className="subnav"><Link href="/cases">← 설치사례 목록</Link><Link href="/inquiry">견적 문의</Link></div>
    <section className={item.image?'detail-hero has-image':'detail-hero'}><div><small><MapPin/> {item.place}</small><h1>{item.title}</h1><p>{item.summary}</p><span>{item.status}</span></div>{item.image&&<Image unoptimized src={item.image} alt={`${item.title} 설치 구성 예시`} width={1200} height={800} priority/>}</section>
    ...
    {product&&<section className="related"><h2>이 구성에 사용되는 제품</h2><div><Link href={`/products/${product.slug}`}><span>{product.category}</span><b>{product.name}</b><ArrowRight/></Link></div></section>}
