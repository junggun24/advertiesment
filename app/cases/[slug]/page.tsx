...
async function storedCase(slug: string) {
  const records = await getContentRecords<CaseStudy>('case');
...
export async function generateMetadata({
  ...
  const saved = await storedCase(slug);
  const item = saved?.item ?? getCase(slug);
  return item
    ? {
        title: item.title,
        ...
        ...(item.image
          ? {
              openGraph: {
                ...
                description: item.summary,
                type: 'article',
                images: [{ url: item.image, alt: `${item.title} 설치 이미지` }],
              },
              twitter: {
...
export default async function CaseDetail({
  ...
  const item = saved?.item ?? getCase(slug);
  if (!item) notFound();
  const storedProducts = await getContentItems<Product>('product');
  const product =
    storedProducts.find((value) => value.slug === item.productSlug) ??
    getProduct(item.productSlug);
  const facts = [
    ['설치 일자', item.installedAt],
    ['고객·시설 유형', item.clientType],
    ['설치 지역', item.region],
    ['적용 모델', item.modelName],
    ['설치 기간', item.projectDuration],
  ].filter((entry): entry is [string, string] => Boolean(entry[1]));
  const sources = safeSources(item.sourceUrls);
  const published = item.installedAt ?? saved?.record.created_at?.slice(0, 10);
    ...
    '@graph': [
      {
        ...
        author: {
          '@type': 'Organization',
          name: item.author || '오아이씨코리아',
        },
        publisher: { '@type': 'Organization', name: '주식회사 오아이씨코리아' },
        about: product
          ? {
              '@type': 'Product',
              name: product.name,
  ...
  return (
    <main className="detail-page">
      <div className="subnav">
        <Link href="/cases">← 설치사례 목록</Link>
        <Link href="/inquiry">견적 문의</Link>
      </div>
      <section className={item.image ? 'detail-hero has-image' : 'detail-hero'}>
        <div>
          <small>
            <MapPin /> {item.place}
          </small>
      ...
      {item.body && (
        <article
          ...
          dangerouslySetInnerHTML={{ __html: prepareEditorHtml(item.body) }}
        />
      )}
      ...
      <section className="case-detail">
        ...
        <div>
          <small>연결 제품</small>
          <strong>{product?.name ?? '제품 정보 확인 중'}</strong>
        </div>
        <div>
          <small>상세 자료</small>
          <strong>
        ...
        <section className="fact-section">
          ...
          <dl>
            ...
            {item.constraints?.length && (
              <div>
                <dt>현장 제약과 대응</dt>
                <dd>{item.constraints.join(', ')}</dd>
              </div>
            )}
          </dl>
        </section>
      )}
      {(item.author || item.reviewer || modified || sources.length > 0) && (
