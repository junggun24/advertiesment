...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const p=getProduct(slug);return p?{title:p.name,description:p.summary,alternates:{canonical:`/products/${slug}`},openGraph:{images:[{url:p.image}]}}:{title:'제품을 찾을 수 없습니다.'}}
...
export default async function ProductDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="detail-page">
    ...
    <section className="detail-hero has-image"><div><small>{product.category}</small><h1>{product.name}</h1><p>{product.summary}</p><span>{product.status}</span></div><img src={product.image} alt={`${product.name} 제품 이미지`}/></section>
