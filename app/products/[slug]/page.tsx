...
export default async function ProductDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="detail-page">
    <div className="subnav"><Link href="/products">← 제품 목록</Link><Link href="/inquiry">견적 문의</Link></div>
    <section className={product.image?'detail-hero has-image':'detail-hero'}><div><small>{product.category}</small><h1>{product.name}</h1><p>{product.summary}</p><span>{product.status}</span></div>{product.image&&<Image unoptimized src={product.image} alt={`${product.name} 제품 이미지`} width={1200} height={800} priority/>}</section>
    ...
    <section className="related"><h2>관련 설치 구성</h2>{related.length?<div>{related.map(item=><Link key={item.slug} href={`/cases/${item.slug}`}><span>{item.place}</span><b>{item.title}</b><ArrowRight/></Link>)}</div>:<p>관련 설치사례를 준비 중입니다.</p>}</section>
