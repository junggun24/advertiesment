...
export default async function InfoPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  const stored=await getContentItems<{tag:string;question:string;answer:string}>('faq');
  const articles=stored.length?stored.map(item=>({tag:item.tag,q:item.question,a:item.answer})):fallbackArticles;
