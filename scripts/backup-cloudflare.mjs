...
try {
  ...
  wrangler([
    'd1', 'export', sourceDatabase, '--remote', '--config', 'wrangler.jsonc',
    '--output', d1File, '--skip-confirmation',
  ]);
  wrangler([
    'r2', 'object', 'put', `${backupBucket}/${prefix}/d1.sql`, '--remote',
    '--config', 'wrangler.jsonc', '--file', d1File, '--content-type',
    'application/sql', '--force',
  ]);

  const queryOutput = wrangler([
    'd1', 'execute', sourceDatabase, '--remote', '--config', 'wrangler.jsonc',
    '--json', '--command',
  ...
  for (const asset of assets) {
    ...
    wrangler([
      'r2', 'object', 'get', `${sourceBucket}/${asset.object_key}`, '--remote',
      '--config', 'wrangler.jsonc', '--file', localFile,
    ]);
    const bytes = readFileSync(localFile);
    const sha256 = createHash('sha256').update(bytes).digest('hex');
    wrangler([
      'r2', 'object', 'put', `${backupBucket}/${prefix}/r2/${asset.object_key}`,
      '--remote', '--config', 'wrangler.jsonc', '--file', localFile,
      '--content-type', asset.content_type || 'application/octet-stream', '--force',
    ]);
  ...
  wrangler([
    'r2', 'object', 'put', `${backupBucket}/${prefix}/manifest.json`, '--remote',
    '--config', 'wrangler.jsonc', '--file', manifestFile,
    '--content-type', 'application/json', '--force',
  ]);
  console.log(JSON.stringify({ ok: true, prefix, assets: manifestAssets.length }));
} finally {
