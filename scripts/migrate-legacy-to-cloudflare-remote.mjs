...
for (const row of content) {
  const contentData = row.type === 'seo'
    ? { ...row.data, siteUrl: publicSiteUrl }
    : row.data;
  statements.push(
    upsert('content_items', contentColumns, [
      Number(row.id), row.type, row.slug, row.title, json(contentData, {}),
...
const temporaryDirectory = mkdtempSync(join(tmpdir(), 'oic-cloudflare-migrate-'));
const sqlFile = join(temporaryDirectory, 'legacy-data.sql');
try {
  writeFileSync(sqlFile, `${statements.join('\n')}\n`, { mode: 0o600 });
  runWrangler([
    'd1','execute','oic-korea-db','--remote','--config','wrangler.jsonc','--file',sqlFile,
  ]);
  ...
  if (!skipR2) {
    for (const asset of assets) {
      ...
      runWrangler(
        [
          'r2','object','put',`${remoteR2Bucket}/${asset.object_key}`,'--remote',
          '--config','wrangler.jsonc','--pipe','--content-type',
          asset.content_type || object.ContentType || 'application/octet-stream','--force',
        ],
